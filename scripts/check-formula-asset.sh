...
#
# The release workflow generates Formula/brightfield.rb wholesale and pushes it
# to the tap. Until this script existed, the checksums in that file came from
...
# stranger's machine at `brew install`. The Intel block is the one that matters
# most: it is a cross-compile no runner in this repository can execute, so no
# other step in the release touches it after packaging.
#
...
#           the target architecture; see the comment above the `update-homebrew`
#           job in .github/workflows/release.yml for where this repository
#           stands on the Intel side of that.
#
# Usage:
#
...
# clean. The release workflow passes 2, one per darwin target.
#
# Exit codes:
...
#      passed.
#
# Its own regression test is scripts/check-formula-asset-selftest.sh, which runs
# on every pull request via .github/workflows/public-hygiene.yml. This script
# needs published release assets, so it can only run on a tag; the self-test is
# what keeps it honest in between.
set -uo pipefail
...
# the generator made and agree with it.
# ---------------------------------------------------------------------------
declare -a URLS=()
declare -a SHAS=()
declare -a LINES=()
