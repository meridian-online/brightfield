...
TASKREF="$(printf '%s-%s' "$_task" 42)"
TICKETREF="$(printf '%s%s' "$_t" 48)"
TICKETREF_PAREN="$(printf '%s%s' "$_t" 7)"
...
check_violation "task id in a body" planning-task-id \
	"$(printf 'Add locale detection\n\nTracked as %s.' "$TASKREF")"
check_violation "bare ticket ref" bare-ticket-ref \
	"$(printf 'Shipped under %s last week.' "$TICKETREF")"
check_violation "parenthesised ticket ref" bare-ticket-ref \
	"$(printf 'Shipped (%s) last week.' "$TICKETREF_PAREN")"
...
check_violation "spec AC id, prefixed" spec-ac-id \
...
check_silent "an upper-case letter and a single digit" \
...
check_silent "a milestone id as a URL path segment" \
	"$(printf 'See https://example.com/%s-%s/spec for details.' m 14)"
...
check_silent "a Rust generic parameter that looks like a ticket ref" \
	"$(printf 'fn pair<%s1, %s22>(a: %s22, b: %s22) -> %s22 { b }' "$_t" "$_t" "$_t" "$_t" "$_t")"
check_silent "an ISO timestamp ending in a capital T plus digits" \
	"Freeze the sample at 2026-07-20T10:30:00Z."
check_silent "a hex digest containing the letters ac" \
	"Pin the lockfile to rev dedcac1."
check_silent "TOML array-of-table headers, not vault wikilinks" \
	"Add [[bin]] and [[test]] targets to Cargo.toml"
...
d="$(new_repo "$(printf 'Close the %s eval-corpus work' "$MILESTONE")")"
perl -0pi -e 's/\Qm-[0-9]{2,}\E/m-[0-9]{2,}(/' "$d/scripts/public-hygiene-rules.txt"
out="$(run_gate "$d")"
rc=$?
