...
def defines(crate: str, symbol: str) -> bool:
    ...
    produced on its first whole-tree run (5 of 7 hits were wrong):
      ...
      2. a MODULE or TEST FILE — "`brightfield-sql`'s `sampling` tests" names
         `crates/brightfield-sql/tests/sampling.rs`, not an item
      3. a reserved STRING LITERAL — "`brightfield-sql`'s `__bf_count` alias"
