...
# nobody's job. They were the surface a leak actually used: a private planning
# identifier reached this repository once in a pull request body and once in a
# commit message, on the same day, and the tracked-file gate reported clean both
# times because it never reads either surface. Nothing was looking.
#
# This repository runs this gate over NEW history only -- see --range below --
# so it carries no scripts/public-hygiene-accepted-history.txt and declares no
# pre-existing exceptions. The mechanism below still supports one, for a future
# repository or a future full-history sweep here; its absence is a scope
# decision, not an oversight, and it means every entry in the DECLARED SET
# described below is empty right now, so every observed match is a violation.
#
...
#   * branch names,
#   * commits reachable only from a range this gate was never asked to scan,
#   * issues, releases, and everything else on the forge.
# A pull request body edited after the last run IS covered, but by a second
# trigger rather than by this file: .github/workflows/pr-text-hygiene.yml
# fires on `edited` and calls this file again with the fresh text.
#
# ---------------------------------------------------------------------------
...
#
# ---------------------------------------------------------------------------
# WHY A PUBLISHED PULL REQUEST BODY REVISION CANNOT BE FIXED EITHER
# ---------------------------------------------------------------------------
# Editing a pull request's body after this gate has already scanned it does not
# undo the exposure. The forge retains every prior revision of an issue or pull
# request body and serves it through its API to any reader with read access to
# the repository, so an identifier that was ever in a saved revision stays
# retrievable by anyone who asks for the edit history, even after the visible
# body is clean.
#
# Removing an already-retained revision is not something a git operation can do
# — it lives on the forge, not in this repository — and the forge does not offer
# a self-service way to delete one revision out of an issue's or pull request's
# edit history; the request would have to go to the forge's own support, which
# is an authenticated, irreversible act outside this repository's control. This
# gate does not perform that request and does not decide whether it should be
# made instead of accepting the exposure as a recorded fact — it only stops the
# next one, the same way the commit-message half of this file cannot rewrite a
# commit that already shipped and only refuses the next one.
# ---------------------------------------------------------------------------
