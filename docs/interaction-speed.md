...
It holds the table's row set too, so deleting the `raw scatter, two views` row —
the row-level half of the comparison — fails there rather than leaving this page
claiming only the fast half. And it reads every `benchmarks/results/*.md` and
`*.json` path written on this page, in prose or as a link target, and fails if
one is missing or names a different run from that record. The rest of the prose
here it does not check.
