...
//! belongs to a UI framework: an egui shell owns plain cells and requests a
//! repaint from its frame context; the retired gpui shell wrapped each
//! `ChartState` in an `Entity` and notified through an `App`; a headless
//! test can hold the state directly and ignore repaints entirely. This
//! trait is that seam — the coordinator (and any other chart logic) is
//! written against it and names no host type, so a new shell drops in by
//! implementing ONE trait without touching the logic again.
//!
...
//!   reactivity. A host with globally reachable cells uses
...
//!   *mechanism* stays with the host (egui: request-repaint;
...
//!   table before servicing a rebuild loop. Handles are expected to be
//!   cheap references to shared cells, not owners.
...
pub trait ReactiveHandle: Clone {
    ...
    /// as (an egui host: its texture id).
    type Surface;

    /// The host context threaded through every update (a host whose cells
