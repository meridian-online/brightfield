...
mod tests {
    ...
    fn dfconf_preflight_reports_unimplemented_mark_only() {
        // Preflight records only non-Implemented marks. `voronoi` is genuinely
        // unimplemented (no renderer/lowerer), so it appears; this captures the
        // spirit: one unimplemented mark → one entry naming it. (geo is now
        // Implemented and would be omitted — the swap stand-in is voronoi.)
        let spec = parse("plot:\n  - mark: voronoi\n    data: { from: t }\n");
        let report = preflight(&spec);
        assert!(report.entries.iter().any(|e| matches!(
            e.identity,
            ComponentIdentity::Mark(brightfield_spec::MarkKind::Voronoi)
        )));
    }
